...
const API = {
  ...
  async list(table, params = {}) {
    ...
    if (params.sort) url += `&order=${params.sort}.desc`;
...
async function xlsxDownload(wb, fileName) {
  if (typeof XLSX === 'undefined') {
    try { await LibLoader.load('xlsx'); }
    catch(e) { Toast.error('엑셀 라이브러리 로드 실패.'); return; }
  }
  try {
...
const Cache = {
  ...
  async get(key, fetcher, ttl=180000) {
    ...
    this._pending[key] = (async()=>{
      try { const data=await fetcher(); this._store[key]={data,at:Date.now()}; return data; }
      finally { delete this._pending[key]; }
    })();
...
const ClientSearchSelect = (() => {
  ...
  function _render(wid) {
    ...
    wrap.innerHTML=`<div class="cs-root" style="position:relative">${val.id
      ?`<div class="cs-selected-box form-control" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;padding:6px 10px;min-height:38px;user-select:none" onclick="ClientSearchSelect._openSearch('${wid}')">
          <span style="font-size:13px;font-weight:500">${Utils.escHtml(val.name)}</span>
          <span style="display:flex;gap:6px;align-items:center">
            <i class="fas fa-exchange-alt" style="color:var(--text-muted);font-size:11px"></i>
            <i class="fas fa-times" style="color:var(--text-muted);font-size:12px" onclick="event.stopPropagation();ClientSearchSelect.clear('${wid}')"></i>
          </span></div>`
      :`<div class="cs-search-box" style="position:relative">
          ...
          <input type="text" class="form-control cs-input-${wid}" id="cs-input-${wid}" style="padding-left:30px;font-size:13px" placeholder="${s.placeholder||'고객사 검색/선택'}"
            oninput="ClientSearchSelect._onInput('${wid}',this.value)"
            onkeydown="ClientSearchSelect._onKey(event,'${wid}')"
            onfocus="ClientSearchSelect._showDropdown('${wid}',this.value)" autocomplete="off" />
          <div id="cs-dropdown-${wid}" class="cs-dropdown" style="display:none;position:absolute;top:calc(100% + 2px);left:0;right:0;background:#fff;border:1px solid var(--border-light);border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,0.12);z-index:3000;max-height:220px;overflow-y:auto"></div>
        </div>`}</div>`;
  }
  function _showDropdown(wid,query) {
    const s=_state[wid]; if(!s) return;
    const ddEl=document.getElementById(`cs-dropdown-${wid}`); if(!ddEl) return;
    const q=(query||'').trim().toLowerCase();
    const filtered=q?s.clients.filter(c=>(c.company_name||c.name||'').toLowerCase().includes(q)):s.clients;
    if(!filtered.length) { ddEl.innerHTML=`<div style="padding:10px 14px;color:var(--text-muted);font-size:13px">검색 결과 없음</div>`; }
    else { ddEl.innerHTML=filtered.slice(0,50).map(c=>{
      ...
      return `<div class="cs-item" data-id="${c.id}" data-name="${Utils.escHtml(lbl)}" style="padding:9px 14px;cursor:pointer;font-size:13px;border-bottom:1px solid #f1f5f9" onmouseover="this.style.background='#f0f7ff'" onmouseout="this.style.background=''" onclick="ClientSearchSelect._pick('${wid}','${c.id}','${lbl.replace(/'/g,"\\'")}')"> ${hi}</div>`;
    }).join(''); }
    ddEl.style.display='';
    if(!s._outsideHandler) {
      s._outsideHandler=(e)=>{ const root=document.getElementById(`cs-dropdown-${wid}`),inp=document.getElementById(`cs-input-${wid}`); if(root&&!root.contains(e.target)&&e.target!==inp) root.style.display='none'; };
  ...
  function _onKey(e,wid) {
    const ddEl=document.getElementById(`cs-dropdown-${wid}`); if(!ddEl||ddEl.style.display==='none') return;
    const items=ddEl.querySelectorAll('.cs-item'); let cur=Array.from(items).findIndex(i=>i.classList.contains('cs-focused'));
    if(e.key==='ArrowDown'){e.preventDefault();if(cur>=0)items[cur].classList.remove('cs-focused');cur=(cur+1)%items.length;items[cur].classList.add('cs-focused');items[cur].style.background='#f0f7ff';items[cur].scrollIntoView({block:'nearest'});}
    else if(e.key==='ArrowUp'){e.preventDefault();if(cur>=0)items[cur].classList.remove('cs-focused');cur=(cur-1+items.length)%items.length;items[cur].classList.add('cs-focused');items[cur].style.background='#f0f7ff';items[cur].scrollIntoView({block:'nearest'});}
    else if(e.key==='Enter'){e.preventDefault();if(cur>=0){const item=items[cur];_pick(wid,item.dataset.id,item.dataset.name);}}
    else if(e.key==='Escape'){ddEl.style.display='none';}
  }
  function _openSearch(wid) {
    const s=_state[wid]; if(!s) return;
    s.selected={id:'',name:''}; _render(wid);
  ...
  function _pick(wid,id,name) {
    ...
    _render(wid); if(typeof s.onSelect==='function') s.onSelect(id,name);
  }
  return {
    ...
    clear(wid) {
      ...
      _state[wid].selected={id:'',name:''}; _render(wid);
...
function setupMenuByRole(session) {
  ...
  const canViewAll=Auth.canViewAll(session);
...
async function updateApprovalBadge(session,force=false) {
  ...
  try {
    const r=await Cache.get('time_entries_badge_'+session.id,async()=>API.list('time_entries',{limit:500}),120000);
    if(r&&r.data){
      let count=0;
...
async function migrateDirectorsToAdmin() {
  ...
  try {
    ...
    if(!targets.length){ console.log('[Migration] 변경 대상 없음'); return; }

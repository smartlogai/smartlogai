...
function _initQuill() {
  ...
  _quill.root.addEventListener('paste', function(e) {
    ...
    if (htmlData && htmlData.includes('<table')) {
      ...
      // 표 스타일 인라인 적용 (테두리, 패딩 등)
      const cleanHtml = _injectDescTableStyle(htmlData);

      // ★ DOM 직접 주입 — Delta 변환 없이 표 구조 그대로 보존
      const editor = _quill.root; // .ql-editor element
      ...
      nodes.forEach(n => {
        if (insertBefore && editor.contains(insertBefore)) {
          editor.insertBefore(n.cloneNode(true), insertBefore);
        } else {
          editor.appendChild(n.cloneNode(true));
        }
      });
...
function _syncQuillToHidden() {
  ...
  if (hidMd && _turndown && finalHtml) {
    try { hidMd.value = _turndown.turndown(finalHtml); }
      ...
      hidMd.value = _quill.getText().trim();
    }
...
const _KW_EXAMPLES = {
  '품목분류': {
    query:  '예: 스마트워치+HS 품목분류 쟁점, 복합기계+관세율표 해석',
    reason: '예: 기본 통칙 적용, GRI 3(b) 본질적 특성, 주(Note) 해석'
  },
  '과세가격': {
    query:  '예: 수입자+특수관계 Management Fee 가산여부, 로열티+과세가격 포함여부',
    reason: '예: 거래가격 인정, 특수관계 영향 없음, 관세법 제30조 적용'
  },
  '원산지판정': {
    query:  '예: 중국산 원자재+완전생산기준 충족여부, ASEAN FTA 원산지 결정기준',
    reason: '예: 세번변경기준(CTH) 충족, 부가가치기준 60% 초과, 완전생산기준'
  },
  '전략물자': {
    query:  '예: 탄소섬유+전략물자 해당여부, 레이저장비+수출허가 대상 확인',
    reason: '예: 통제번호(ECCN) 미해당, EAR99 해당, 수출허가 면제'
  },
  '요건대상': {
    query:  '예: 의료기기+수입요건 해당여부, 화장품+KC인증 대상 확인',
    reason: '예: HS코드 기준 요건 미해당, 면제조건 충족, 시험성적서 필요'
  },
  'FTA활용': {
    query:  '예: 한-EU FTA+원산지증명서 발급, RCEP 협정관세율 적용방법',
    reason: '예: 세번변경기준 충족, 원산지신고서 발급, 직접운송 요건 충족'
  },
  '관세환급': {
    query:  '예: 수출제품+수입원자재 간이정액환급, 개별환급 소요량 산정',
    reason: '예: 소요량 산정 기준 충족, 환급신청기한 내, 관세환급특례법 적용'
  },
  '기타': {
    query:  '예: 수입통관+세관 심사 대응, AEO 인증 요건 확인',
    reason: '예: 관련 법령 해석, 행정해석 참고, 실무 관행 적용'
  }
};
...
function _updateKwExamples() {
  ...
  if (rEl) rEl.textContent = exampleSet ? exampleSet.reason : '예: 관련성 불충족, 거래조건불충족, 과세가격가산불필요';
}
...
async function checkTimeOverlap(newStart, newEnd, excludeId = '') {
  try {
    ...
    // 본인 것만, 삭제 안 된 것만, 수정 시 자신 제외 (String 비교 — 타입 불일치 방지)
    const mine = entries.filter(e =>
      String(e.user_id) === String(session.id) &&
      !e.deleted &&
      e.id !== excludeId &&
...
async function _doSaveEntry(status, approverInfo) {
  ...
  try {
    const entryData = {
      ...
      team_id:   teamId,
      team_name: teamName,
      client_id:   catType === 'client' ? clientId : '',
      client_name: catType === 'client' ? clientName : '',
    ...
    if (_editEntryId) {
      ...
      entry = await API.create('time_entries', entryData);
    }
  ...
  try {
    ...
    infoGrid.innerHTML = `
      ...
      <div>
        ...
        <div style="font-size:13px;font-weight:600;color:var(--text-primary)">${entry.client_name || '<span style="color:var(--text-muted)">내부업무</span>'}</div>
      </div>
      <div>
    ...
    descLabel.style.cssText = 'font-size:11px;color:var(--text-muted);margin-bottom:6px;font-weight:600;display:flex;align-items:center;gap:5px';
    descLabel.innerHTML = '<i class="fas fa-align-left"></i> 수행 내용';
    body.appendChild(descLabel);

    const descBox = document.createElement('div');
    const _isHtmlDesc = entry.work_description && /[<>]/.test(entry.work_description);
    descBox.className = _isHtmlDesc ? 'arch-desc-view' : '';
    descBox.style.cssText = 'background:#f8fafc;border:1px solid var(--border-light);border-radius:8px;padding:12px 14px;font-size:13px;line-height:1.7;color:var(--text-primary);min-height:64px;margin-bottom:16px;overflow-x:auto;word-break:break-word';
    if (!entry.work_description) {
      descBox.innerHTML = '<span style="color:var(--text-muted)">입력된 내용이 없습니다.</span>';
    } else if (_isHtmlDesc) {
      // HTML 형식(표 등 포함) → innerHTML로 렌더링 (arch-desc-view CSS 표 스타일 적용)
      descBox.innerHTML = entry.work_description;
    } else {
      // 순수 텍스트
      descBox.style.whiteSpace = 'pre-wrap';
      descBox.textContent = entry.work_description;
    }
...
async function editEntry(id) {
  try {
    ...
    // ⑩ 수행내용 세팅 — Quill 에디터에 HTML 복원
    const descHtml = entry.work_description || '';
    if (_quill) {
      // HTML이 있으면 innerHTML로 직접 복원 (표 구조 보존)
...
async function exportEntriesToExcel() {
  ...
  try {
    ...
    const r = await API.list('time_entries', { limit: 500 });
    console.log('[Excel] step1 result:', r);
    let entries = (r && r.data) ? r.data : (Array.isArray(r) ? r : []);

    // 역할별 필터 (String 비교 — 타입 불일치 방지)
    ...
    const rows = entries.map((e, i) => {
      ...
      return {
        ...
        '수행내용':  (e.work_description || '').replace(/<[^>]+>/g, '').replace(/&nbsp;/g,' ').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&').trim(),
